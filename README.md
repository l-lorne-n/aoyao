...
10. 增加地址分类、手动病历编号、同地址编号唯一校验和组合检索。
11. 增加批量选择病历并导出纸质病历本风格 PDF。
12. 增加按日期浏览历史病历的时间线页面，可从小卡片跳回完整病历。
13. 增加手柄导航、手柄录音、手柄保存、手柄删除和选择层操作。
14. 增加垃圾桶软删除、复原和彻底删除。
15. 增加未保存修改保护，覆盖刷新、关闭、切换病历、新建病历和页面内跳转。
16. 配置 `.gitignore`，避免提交密钥、数据库、录音样本、PDF 导出文件和患者信息。
